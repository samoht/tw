...
let test_gradient_direction () =
  ...
  check string "bg-gradient-to-b" "bg-gradient-to-b" (Tw.Core.pp t)
...
let test_gradient_colors () =
  ...
  check string "from-red-500" "from-red-500" (Core.pp from);
  check string "via-blue-600" "via-blue-600" (Core.pp via);
  check string "to-green-500" "to-green-500" (Core.pp to_)
