...
(* Tailwind substitutes [--alpha(<color>/<alpha>)] wherever it stands in an
   arbitrary value, as it does [--spacing()], writing the [color-mix()] it
   denotes in its place: the call was read only when it was the whole value. The
   alpha is scaled the way {!Tw.Parse.alpha_call} scales it, a whole one leaves
   the colour itself, and a call missing either half stays as written for the
   refusal that follows. The same bytes inside a quoted string are a string
   literal. *)
let test_alpha_fn_expands_in_place () =
  let expands input expected =
    Alcotest.(check string)
      input expected
      (Tw.Parse.decode_arbitrary_value input)
  in
  expands "--alpha(red/50%)" "color-mix(in oklab, red 50%, transparent)";
  expands "0_0_0_1px_--alpha(red/0.5)"
    "0 0 0 1px color-mix(in oklab, red 50%, transparent)";
  expands "--alpha(red/1)" "red";
  expands "--alpha(var(--c)/var(--o))"
    "color-mix(in oklab, var(--c) var(--o), transparent)";
  expands "linear-gradient(--alpha(red/50%),blue)"
    "linear-gradient(color-mix(in oklab, red 50%, transparent),blue)";
  expands "--spacing(4)_--alpha(red/50%)"
    "calc(var(--spacing) * 4) color-mix(in oklab, red 50%, transparent)";
  expands "--alpha(--alpha(red/50%)/50%)"
    "color-mix(in oklab, color-mix(in oklab, red 50%, transparent) 50%, \
     transparent)";
  expands "--alpha(oklch(1_0_0_/_50%)/20%)"
    "color-mix(in oklab, oklch(1 0 0 / 50%) 20%, transparent)";
  expands {|'--alpha(red/50%)'|} {|'--alpha(red/50%)'|};
  expands "--alpha(red)" "--alpha(red)";
  expands "--alpha(red/)" "--alpha(red/)"
  ...
  Alcotest.
    [
      ...
      test_case "--alpha() halves and a bare number alpha" `Quick
        test_alpha_call;
      test_case "--alpha() expands in place" `Quick
        test_alpha_fn_expands_in_place;
