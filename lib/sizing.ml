...
type Utility.base += Sizing of t

let wrap x = Sizing x
let unwrap = function Sizing x -> Some x | _ -> None
...
let aspect_ratio w h = base (Aspect_ratio (w, h))
