...
let ring_offset_shadow () =
  let open Css in
  let spread : length = Var (Var.reference Effects.ring_offset_width_var) in
  let color : color = Var (Var.reference Effects.ring_offset_color_var) in
  shadow ~inset:false
    ~inset_var:(Var.name Effects.ring_inset_var)
    ~inset_var_no_fallback:true ~h_offset:Zero ~v_offset:Zero ~blur:Zero ~spread
    ~color ()
    ...
    Calc
      Calc.(
        add
          ...
          (var ~default:offset_width_default
             (Var.name Effects.ring_offset_width_var)))
  in
  let color : color = Var (var_ref (Var.name Effects.ring_color_var)) in
  shadow ~inset:false
    ~inset_var:(Var.name Effects.ring_inset_var)
    ~inset_var_no_fallback:true ~h_offset:Zero ~v_offset:Zero ~blur:Zero ~spread
    ~color ()
